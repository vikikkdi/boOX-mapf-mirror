...
#ifndef __AGENT_SMT_CBS_H__
#define __AGENT_SMT_CBS_H__
...
#include "boOX_core/boOX_graph.h"
...
// sConfiguration
	...
	virtual sResult from_vector(const std::vector<std::pair<int, int> > &vec, const int&x,  const int &y);
...
*/
	...
	virtual sResult from_vector(std::vector<std::pair<int, int> > obstacles, std::vector<std::pair<int, int> > goals, std::vector<std::pair<int, int> > starts, int x, int y, std::vector<std::vector<int> > graph);
...
*/
	...
	*/
	...
	sInt_32 construct_HamiltonianMDD(sInt_32     max_total_cost,
					 MDD_vector &MDD,
					 sInt_32    &extra_cost,
					 MDD_vector &extra_MDD);
...
// sSolution
	...
	virtual sResult to_vector(std::pair<float, std::vector<std::vector<std::tuple<int, int, int> > > > *solution, int n, int x, int y);
