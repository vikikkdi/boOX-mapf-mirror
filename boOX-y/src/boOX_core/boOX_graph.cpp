...
//			m_Edges.back().m_Conflicts.push_back(sEdge::Conflict(x_id, y_id));
    ...
    sResult sUndirectedGraph::from_vector(std::vector<std::pair<int, int> > obstacles, std::vector<std::pair<int, int> > goals, std::vector<std::pair<int, int> > starts, int x, int y, std::vector<std::vector<int> > graph){
		m_Vertices.clear();
		m_Edges.clear();
		for(int i=0; i<x; i++){
			for(int j=0; j<y; j++){
				int xy = i*y + j;
				add_Vertex();//m_Vertices.push_back(sVertex(xy));
			}
		}
		...
		for(int i = 0; i < x; ++i){
			for(int j = 0; j < y; ++j){
				...
				if(processed_obstacles.find(xy) != processed_obstacles.end()){
					...
					//Not an Obstacle
					if(j+1 < y && processed_obstacles.find((i*y)+(j+1)) == processed_obstacles.end()){
						int right = (i*y) + (j+1);
						add_Edge(xy, right);
					}
					if(i+1 < x && processed_obstacles.find(((i+1)*y)+(j)) == processed_obstacles.end()){
						int down = ((i+1)*y) + j;
						add_Edge(xy, down);
					}
				}
